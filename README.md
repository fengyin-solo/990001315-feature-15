...
- **区间对比**：支持今天、近七天、自定义区间的新增/通过/拒绝数据对比，异常变化自动提示
...
首页提供数据区间对比模块，帮助运营人员了解留言趋势：

1. **区间切换**：支持今天、近七天、自定义区间（最长90天）
2. **对比指标**：新增、通过、被拒绝三个指标与上一周期的对比
3. **异常提示**：变化幅度超过50%时自动标记为异常
4. **容错机制**：
   - 缺少历史数据时显示提示
   - 区间过大或计算异常时保留上次结果
   - 未完成区间会明确标注
5. **状态保持**：刷新页面或重新进入时保留所选区间

### 数据库迁移

如果是从旧版本升级，需要执行迁移脚本添加审核时间字段：

```bash
mysql -u root -p community_board < database/migration_add_audited_at.sql
```
